        ...
        <div className="flex items-center gap-2">
          ...
          <div className="relative" ref={accountMenuRef}>
            <button
              ...
              className="flex items-center gap-2 p-1.5 md:p-2 rounded-lg hover:bg-muted transition-smooth"
              aria-label="Account menu"
            >
              {user ? (
                <>
                  <div className="w-8 h-8 md:w-9 md:h-9 bg-primary text-primary-foreground rounded-full flex items-center justify-center font-semibold text-xs md:text-sm overflow-hidden border-2 border-primary/20">
                    {user?.photoUrl ? (
                      <img src={user.photoUrl} alt={user.name} className="w-full h-full object-cover" />
                    ) : (
                      <span>{user?.name?.split(' ')?.map((n) => n?.[0])?.slice(0, 2).join('') || 'U'}</span>
                    )}
                  </div>
                  <Icon name="ChevronDown" size={16} className={`text-muted-foreground hidden sm:block transition-transform duration-200 ${isAccountMenuOpen ? 'rotate-180' : ''}`} />
                </>
              ) : (
                <div className="flex items-center gap-2">
                  <div className="w-8 h-8 md:w-9 md:h-9 bg-muted text-muted-foreground rounded-full flex items-center justify-center border border-border">
                    <Icon name="User" size={18} />
                  </div>
                  <span className="text-sm font-medium text-foreground hidden sm:block">Sign In</span>
                </div>
              )}
            </button>

            {isAccountMenuOpen && (
              <div className="absolute right-0 mt-2 w-64 bg-popover border border-border rounded-lg shadow-elevation-4 overflow-hidden z-200 animate-in fade-in slide-in-from-top-2 duration-200">
                {user ? (
                  <>
                    <div className="p-4 bg-muted/30 border-b border-border">
                      <p className="font-semibold text-popover-foreground truncate">{user?.name || 'User'}</p>
                      <p className="text-xs text-muted-foreground truncate mt-0.5">{user?.email || ''}</p>
                    </div>
                    <div className="py-2">
                      <button onClick={() => { setIsAccountMenuOpen(false); navigate('/profile'); }} className="w-full px-4 py-2.5 text-left text-sm text-popover-foreground hover:bg-muted transition-smooth flex items-center gap-3">
                        <Icon name="User" size={16} className="text-primary" />
                        Profile Settings
                      </button>
                      <button onClick={() => { setIsAccountMenuOpen(false); if (location?.pathname === '/profile') { window.dispatchEvent(new CustomEvent('profile-focus', { detail: { focus: 'passports' } })); } else { navigate('/profile', { state: { focus: 'passports' } }); } }} className="w-full px-4 py-2.5 text-left text-sm text-popover-foreground hover:bg-muted transition-smooth flex items-center gap-3">
                        <Icon name="FileText" size={16} className="text-primary" />
                        Passport Management
                      </button>
                      <button onClick={() => { setIsAccountMenuOpen(false); if (location?.pathname === '/profile') { window.dispatchEvent(new CustomEvent('profile-focus', { detail: { focus: 'preferences' } })); } else { navigate('/profile', { state: { focus: 'preferences' } }); } }} className="w-full px-4 py-2.5 text-left text-sm text-popover-foreground hover:bg-muted transition-smooth flex items-center gap-3">
                        <Icon name="Settings" size={16} className="text-primary" />
                        Preferences
                      </button>
                    </div>
                    <div className="border-t border-border py-2 bg-muted/10">
                      <button onClick={handleLogout} className="w-full px-4 py-2.5 text-left text-sm text-error hover:bg-error/10 transition-smooth flex items-center gap-3">
                  ...
                  <div className="p-2 space-y-1">
                    <button onClick={() => { setIsAccountMenuOpen(false); navigate('/user-login'); }} className="w-full px-4 py-3 text-left text-sm font-medium text-primary hover:bg-primary/5 rounded-md transition-smooth flex items-center gap-3">
                      <Icon name="LogIn" size={18} />
                      Log In
                    </button>
                    <button onClick={() => { setIsAccountMenuOpen(false); navigate('/user-registration'); }} className="w-full px-4 py-3 text-left text-sm font-medium text-white bg-primary hover:bg-primary/90 rounded-md transition-smooth flex items-center gap-3 shadow-sm">
